...
from DataManager.forms.TipallForm import TipallForm
from DataManager.models.tipall import Tipall
from django.shortcuts import get_object_or_404
from ..functions import *

def tipall(request, model=Tipall, modelform=TipallForm, template='DataManager/mainform.html', url_name='tipall'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codtal', 'destal')}
