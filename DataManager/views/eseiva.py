...
from DataManager.forms.EseivaForm import EseivaForm
from DataManager.models.eseiva import Eseiva
from django.shortcuts import get_object_or_404
from ..functions import *

def eseiva(request, model=Eseiva, modelform=EseivaForm, template='DataManager/mainform.html', url_name='eseiva'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codese',)}
