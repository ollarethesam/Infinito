...
from DataManager.forms.ZoneForm import ZoneForm
from DataManager.models.zone import Zone
from django.shortcuts import get_object_or_404
from ..functions import *

def zone(request, model=Zone, modelform=ZoneForm, template='DataManager/mainform.html', url_name='zone'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codzon', 'deszon')}
