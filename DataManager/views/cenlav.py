...
from DataManager.forms.CenlavForm import CenlavForm
from DataManager.models.cenlav import Cenlav
from django.shortcuts import get_object_or_404
from ..functions import *

def cenlav(request, model=Cenlav, modelform=CenlavForm, template='DataManager/mainform.html', url_name='cenlav'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codcen', 'descen')}
