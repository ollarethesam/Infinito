...
from DataManager.forms.CautraForm import CautraForm
from DataManager.models.cautra import Cautra
from django.shortcuts import get_object_or_404
from ..functions import *

def cautra(request, model=Cautra, modelform=CautraForm, template='DataManager/mainform.html', url_name='cautra'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codcau', 'descau')}
