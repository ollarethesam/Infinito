...
from DataManager.forms.SpedizForm import SpedizForm
from DataManager.models.spediz import Spediz
from django.shortcuts import get_object_or_404
from ..functions import *

def spediz(request, model=Spediz, modelform=SpedizForm, template='DataManager/mainform.html', url_name='spediz'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codspe', 'desspe')}
