...
from DataManager.forms.AspestForm import AspestForm
from DataManager.models.aspest import Aspest
from django.shortcuts import get_object_or_404
from ..functions import *

def aspest(request, model=Aspest, modelform=AspestForm, template='DataManager/mainform.html', url_name='aspest'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codasp', 'desasp')}
