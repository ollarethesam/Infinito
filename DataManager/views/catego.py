...
from DataManager.forms.CategoForm import CategoForm
from DataManager.models.catego import Catego
from django.shortcuts import get_object_or_404
from ..functions import *

def catego(request, model=Catego, modelform=CategoForm, template='DataManager/mainform.html', url_name='catego'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codcat', 'descat')}
