...
from DataManager.forms.ValuteForm import ValuteForm
from DataManager.models.valute import Valute
from django.shortcuts import get_object_or_404
from ..functions import *

def valute(request, model=Valute, modelform=ValuteForm, template='DataManager/mainform.html', url_name='valute'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codval', 'desval')}
