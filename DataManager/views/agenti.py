...
from DataManager.forms.AgentiForm import AgentiForm
from DataManager.models.agenti import Agenti
from django.shortcuts import get_object_or_404
from ..functions import *

def agenti(request, model=Agenti, modelform=AgentiForm, template='DataManager/mainform.html', url_name='agenti'):
    if request.method == 'POST':   
        pk_val = request.POST[model._meta.pk.name]
        return save_or_update(model, modelform, request, pk_val)
    if request.method == 'GET':
        form = modelform
        context = {'form': form, 'url_name': url_name, 'ddfields': ('codage', 'desage')}
