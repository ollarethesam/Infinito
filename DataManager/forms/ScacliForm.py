...
class ScacliForm(ModelForm):
    ragsoc = forms.CharField(required=False, label='Ragione Sociale', widget=forms.TextInput(attrs={'class':'ragsoc Client form-control', 'id': False}))
    ...
    class Meta:
        ...
        model = Scacli
        fields = ["numpro", 'descri', "codcli", 'ragsoc', "tipdoc", "datdoc", "numfat", "impfat", "imppag", "codval", 'desval', "codpag", 'despag', "cambio", "datsca", "pagato", "codban", 'desban', "tippag", 'modpag']
        labels = {
        ...
        widgets = {
            'numpro': forms.TextInput(attrs={'class':'numpro Scacli form-control pk l7', 'autocomplete': 'off', 'id': False}),
            'codcli': forms.TextInput(attrs={'class':'codcli Client form-control l5', 'autocomplete': 'off', 'id': False}),
            ...
            'descri': forms.TextInput(attrs={'class':'descri Scacli form-control l35', 'autocomplete': 'off', 'id': False}),
    ...
    def clean_numpro(self, *args, **kwargs):
        ...
        if Scacli.objects.last() is not None:
            if numpro > int(Scacli.objects.last().pk) + 1:
                raise forms.ValidationError(f"Numero progressivo sbagliato, usare {int(Scacli.objects.last().pk) + 1}")
            else:
                return numpro
        else:
            if numpro != 1:
